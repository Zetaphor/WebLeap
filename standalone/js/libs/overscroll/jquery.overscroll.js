...
(function(global, dom, browser, math, wait, cancel, namespace, $, none) {
    ...
    function captureLeapPosition(pointable, position, index) {
        var pos = pointable.offset();
        position.x = pos.left;
        position.y = pos.top;
        position.time = time();
        position.index = index;
    ...
    // starts the drag operation and binds the mouse move handler
    function leapStart(event, pointable) {
        console.log("start");
        var point = $(pointable);
        var data = event.data,
                target = data.target,
        ...
        if (dstart.size() && !dstart.is(data.options.cancelOn)) {
            if (!compat.overflowScrolling) {
                target.data(datakey).dragging = flags.dragging = flags.dragged = false;
                // apply the drag listeners to the doc or target
                $(document).on(events.leapMove, data, leapMove);

            }

            data.position = captureLeapPosition(point, {});
            data.capture = captureLeapPosition(point, {}, settings.captureThreshold);
    ...
    // ends the drag operation and unbinds the mouse move handler
    function leapStop(event, pointable) {
        var point = $(pointable);
        ...
        $(document).unbind(events.leapMove, leapMove);
    ...
    // updates the current scroll offset during a mouse move
    function leapMove(event, pointable) {
        var point = $(pointable);
        var pos = point.offset();
        event.preventDefault();
        ...
        if (options.direction !== 'vertical') {
            target.scrollLeft -= (pos.left - position.x);
        }

        if (data.options.direction !== 'horizontal') {
            target.scrollTop -= (pos.top - position.y);
        }

        captureLeapPosition(point, data.position);

        if (--data.capture.index <= 0) {
            data.target.data(datakey).dragging = flags.dragging = true;
            captureLeapPosition(point, data.capture, settings.captureThreshold);
        }
    ...
    function setup(target, options) {
        ...
        if (sizing.valid) {
            ...
            if (options.enableLeap) {
                $(document).on(events.leapStart, data, leapStart);
                $(document).on(events.leapEnd, data, leapStop);
            }
